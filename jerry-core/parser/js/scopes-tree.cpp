...
scopes_tree_child_scopes_num (scopes_tree tree)
{
  ...
  scopes_tree child_scopes_iter_p = MEM_CP_GET_POINTER (scopes_tree_int, tree->children_list_cp);

  while (child_scopes_iter_p != NULL)
  {
    child_scopes_iter_p = MEM_CP_GET_POINTER (scopes_tree_int, child_scopes_iter_p->next_scope_cp);
                  ...
                  scope_type_t type) /**< scope type */
{
  ...
  if (parent != NULL)
  {
    scopes_tree child_scopes_iter_p = MEM_CP_GET_POINTER (scopes_tree_int, parent->children_list_cp);

    if (child_scopes_iter_p != NULL)
    {
      scopes_tree child_scope_iter_prev_p = NULL;

      while (child_scopes_iter_p != NULL)
      {
        child_scope_iter_prev_p = child_scopes_iter_p;

        child_scopes_iter_p = MEM_CP_GET_POINTER (scopes_tree_int, child_scopes_iter_p->next_scope_cp);
      }

      JERRY_ASSERT (child_scope_iter_prev_p->next_scope_cp == MEM_CP_NULL);

      MEM_CP_SET_NON_NULL_POINTER (child_scope_iter_prev_p->next_scope_cp, tree);
    }
    else
    {
      MEM_CP_SET_NON_NULL_POINTER (parent->children_list_cp, tree);
    }
  }
  ...
  tree->children_list_cp = MEM_CP_NULL;
  tree->instrs_count = 0;
  tree->type = type;
  tree->is_root_scope = (parent == NULL);
...
void
scopes_tree_free (scopes_tree scope_p)
{
  assert_tree (scope_p);
